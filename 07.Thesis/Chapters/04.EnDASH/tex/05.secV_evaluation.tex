...
\begin{figure}[h]
	\captionsetup[subfigure]{width=0.49\linewidth}
	\begin{center}
		\subfloat[\label{fig:chap04:energyWF}Avg. Bitrates in Mbps]{
			\includegraphics[width=0.49\linewidth]{new_results/simres/EnergyWF}
		}
		\subfloat[\label{fig:chap04:energyperbit}Stall Time per segment (in seconds)]{
			\includegraphics[width=0.49\linewidth]{new_results/simres/EnergyPerBitWF}
		}
	\end{center}
	\caption{\label{fig:chap04:energycost}Comparison of energy consumption and energy efficiency between different ABR algorithm and lowest quality video playback.}
\end{figure}
\RC{Apparently it looks like that EnDASH saves energy just by reducing the video quality. Although quality is compromised a bit to save energy, the scheduling is also very important. We can understand this from \fig{\ref{fig:chap04:energycost}}. The \fig{\ref{fig:chap04:energyWF}} clearly shows that over energy consumption of the ABR algorithms without proper segment download schedule takes more or less same even if the video is being played in lowest quality. \fig{\ref{fig:chap04:energyperbit}} depict the energy requirement per bit downloaded data. Here interestingly we can see that energy requirement per bit for lowest quality video is very high. The primary reason is that there is no segment download scheduling available. Energy consumption per bit for EnDASH is comparable with other ABR algorithm although it is slightly less efficient. In the end we can conclude that energy consumption per bit is not that important as compared to the overall energy consumption and video quality.}
