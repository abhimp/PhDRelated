...
\subsection{Energy Efficiency Compared to Lowe Quality Video}
\begin{figure}[h]
	\captionsetup[subfigure]{width=0.49\linewidth}
	\begin{center}
		\subfloat[\label{fig:chap04:energyWF}Total energy consumption per video session]{
			\includegraphics[width=0.49\linewidth]{new_results/simres/EnergyWF}
		}
		\subfloat[\label{fig:chap04:energyperbit}Energy consumption per bit downloaded data]{
			\includegraphics[width=0.49\linewidth]{new_results/simres/EnergyPerBitWF}
		}
	\end{center}
	\caption{\label{fig:chap04:energycost}Comparison of energy consumption and energy efficiency between different ABR algorithms and lowest quality video playback. Here B = BOLA, P = Pensieve, MF = Fast-MPC, and MR = Robust-MPC.}
\end{figure}

\RC{It looks like EnDASH saves energy just by reducing the video quality. Although quality is compromised a bit to save energy, scheduling is also very important. We can understand this from \fig{\ref{fig:chap04:energycost}}. \fig{\ref{fig:chap04:energyWF}} clearly shows that over energy consumption of the ABR algorithms without proper segment download schedule takes more or less the same even if the video is being played in the lowest quality. \fig{\ref{fig:chap04:energyperbit}} depicts the energy requirement per bit downloaded data.}

\RC{Interestingly, we can see that the energy requirement per bit for the lowest quality video is very high. The primary reason is that there is no segment download scheduling available. Energy consumption per bit for EnDASH is comparable with other ABR algorithms, although it is slightly less efficient. We can conclude that energy consumption per bit is not that important compared to overall energy consumption and video quality.}
