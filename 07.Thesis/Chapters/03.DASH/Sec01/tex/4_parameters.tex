\section{Identifying Parameters Involved in YouTube Adaptive Streaming, and Their Interplay}
...
\begin{figure}[!h]
 \centering
  ...
  \label{fig:chap03:rbuf_evol}
\end{figure}
\begin{figure}[!h]
 \centering
  \includegraphics[width=0.7\linewidth]{img/plot_timerange}
  \caption{\small{$itag$ evolution}}
  \label{fig:chap03:timespent}
\end{figure}
\begin{figure}[!h]
 \centering
  \includegraphics[width=0.7\linewidth]{img/plot_seg_comb}
  \caption{\small{Segment length adaptation}}
  \label{fig:chap03:segmentLength}
\end{figure}

{\bf {\em rn}:} Our experiments unveil that $rn$ is non-decreasing for a video session.
%
By observing the sequence of HTTP requests sent by the YouTube client to YouTube server, we conclude that $rn$ is the request number to uniquely identify a DASH video playback request.

{\bf {\em rbuf}:} We plot evolution of $rbuf$ for $4$ videos and the corresponding $itag$ values in Fig.~\ref{fig:chap03:rbuf_evol} with varying link bandwidth.
...
\subsection{Insights into YouTube's bitrate adaptation algorithm}
...
{\bf Segment length adaptation:} As mentioned earlier, YouTube adapts both the video quality and the streaming data rate whenever network conditions change.
