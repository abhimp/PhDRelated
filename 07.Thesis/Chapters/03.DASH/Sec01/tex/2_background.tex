...
\RC{\fig{\ref{fig:packet}} and \fig{\ref{fig:burst}} shows the instantaneous data download rate (bytes per 10ms) from the YouTube server to YouTube client with respect to time and changing link bandwidth, while  \fig{\ref{fig:reso}} indicates the video playback resolution with respect to time. In both the graphs, we show the change in link bandwidth (through the throttling mechanism) with respect to time, using the right hand side Y-axis. From \fig{\ref{fig:packet}}, we can clearly see that download rate increases as we increase the bandwidth and drop when link bandwidth is decreased. \fig{\ref{fig:burst}} shows the same thing by scaling up the time axis, and this figure indicates that in YouTube, data are indeed transferred in bursts. A group of data transferred and buffered at the YouTube client, and then there is a silent zone when no data is transferred. From these two figures it is also clear that the size of bursts are not the same. From these observations, we can conclude that YouTube dynamically changes content length in each burst based on link bandwidth. It transfers content in data bursts, and reduces (increases) the content length when link bandwidth is low (high).}
